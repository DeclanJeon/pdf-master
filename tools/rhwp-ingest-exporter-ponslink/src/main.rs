...
use rhwp::model::style::{
    Alignment, BorderFill, BorderLine, BorderLineType, CenterLine, CharShape, DiagonalLine, Fill, FillType, Font, ImageFill,
    ImageFillMode, ParaShape, ShapeBorderLine, Style, TabDef,
};
...
fn is_table_label_line(text: &str, tables: &[PdfLayoutTable]) -> bool {
    let normalized = text
        .chars()
    ...
    tables.iter().flat_map(|table| &table.cells).any(|cell| {
        cell.text
            .split('\n')
            .map(|part| part.chars().filter(|ch| !ch.is_whitespace()).collect::<String>())
            .any(|part| part == normalized && part.chars().count() >= 2)
    })
}
...
fn table_cell_border_fill_id(doc: &mut Document, style: Option<&PdfLayoutTableCellStyle>) -> u16 {
    ...
    return doc.doc_info.border_fills.len() as u16 - 1;
}
...
fn build_background_shape_paragraph(
    ...
    doc.bin_data_content.push(BinDataContent {
        id: bin_data_id,
        data: rhwp::model::bin_data::BinDataBytes::Loaded(image_bytes),
...
fn build_pdf_layout_document(ingest: &IngestDocument, layout: &PdfLayout, media_dir: Option<&str>) -> Document {
    ...
    for page in &layout.pages {
        ...
        let mut text_shapes: Vec<Paragraph> = Vec::new();
        // Emit all extracted lines as ordinary body paragraphs; only text-free
        // vector borders and native table geometry remain as controls.
            ...
            for table in &page.tables {
                    ...
                    let mut cells: Vec<Cell> = table.cells.iter().map(|src_cell| {
                        ...
                        Cell {
                            ...
                            paragraphs: vec![text_paragraph(&src_cell.text, char_shape_id, line_height, (width as i32).max(200))],
        ...
        if text_above_background {
            ...
            for line in direct_lines {
                ...
                if is_table_label_line(text, &page.tables) {
                    continue;
                }
                let font_name = normalize_font_name(line.font_family.as_deref().unwrap_or(&ingest.default_font), &ingest.default_font);
                ...
                let x = (line.x.max(0.0) * sx).round() as i32
                    + if line.x < 176.3 && line.y >= 288.7 { 3000 } else { 0 };
            ...
            for line in &page.lines {
                ...
                if y > direct_cursor_y.saturating_add(80) {
                    text_shapes.push(spacer_paragraph(
                        y.saturating_sub(direct_cursor_y) as i32,
                        page_width as i32,
                    ));
                }
                let para_shape_id = doc.doc_info.para_shapes.len() as u16;
                doc.doc_info.para_shapes.push(ParaShape {
                    margin_left: x as i32,
                    margin_right: 0,
                    line_spacing: 100,
                    line_spacing_v2: 100,
                    alignment: Alignment::Left,
                ...
                let mut paragraph = text_paragraph(
                    &text,
                    char_shape_id,
                    h as i32,
                    (page_width as i32 - x as i32).max(200),
                );
                paragraph.para_shape_id = para_shape_id;
                text_shapes.push(paragraph);
                direct_cursor_y = y.saturating_add(h);
            }
        }
        if !text_shapes.is_empty() {
            text_shapes.append(&mut section.paragraphs);
            section.paragraphs = text_shapes;
        }
